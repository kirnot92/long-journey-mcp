...
`recall`은 구체적인 사건·조건·결과에 맞는 경험을, `think`는 주제에 관해 축적된 철학·관점·원칙·패턴을 검색한다. 호출 에이전트가 각 의도에 맞는 쿼리를 작성하고 내부 검색·모델 선택은 공유한다. Depth 필터나 가중치를 두지 않으며 높은 depth 반환을 보장하지 않는다. Think에 별도 추론·답변 생성·기억 생성 단계를 추가하지 않는다. 두 도구의 회수 기록은 동일하게 이후 Dream seed가 될 수 있다. 현재 입력·반환·비용·계측 계약은 [운영 안내](05-operations.md#mcp-도구와-결과)를 따른다.
...
`remember(raw)`의 입력은 호출 에이전트가 기억할 가치가 있다고 선택한 **하나의 일관된 경험**이다. 그 경험을 이해하는 데 필요한 주체·상황·조건 등의 맥락을 함께 전달한다. 전체 세션을 필수 입력으로 요구하거나, 문장·발화·observation 하나로 입력을 고정하지 않는다.

### 5.2 depth 0의 의미와 출력 개수

raw는 immutable Source 하나로 보존하고, 그 원문에서 직접 확인할 수 있는 observation을 `0..N`개의 depth-0 Memory로 만든다. Depth 0은 원문에서 내용을 선택하고 최소한으로 정규화한 직접 관찰이다. 원문에 없는 일반화·성향·선호·원인을 추론해 추가하지 않으며, 불확실성·조건·부정을 유지한다. 정보량이 없는 입력에서는 Memory를 만들지 않을 수 있다.

입력 크기와 출력 개수의 상한은 설정값이며, 하나의 경험이 반드시 하나의 observation에 대응해야 하는 것은 아니다. 같은 Source에서 나온 모든 observation은 같은 root를 공유하므로 observation 수가 늘어도 root 수는 1이다.
...
그 외 benchmark 상세 설계와 최초 적용 대상 선정은 Phase 5까지 보류한다. 기억할 경험을 선택하는 입력 원칙에 따라 최초 설계의 session/trajectory 단위 `remember()` replay 방식도 그때 재검토해야 한다.
...
- raw의 크기 제한, 초과 입력 처리 및 출력 개수 상한의 구체적인 설정.
