...
공개 도구는 `remember(raw)`, `recall(query, context?)`, `trace(memory_id)` 세 개다. `remember`에는 호출 에이전트가 기억할 가치가 있다고 선택한 하나의 일관된 경험을 이해에 필요한 맥락과 함께 전달한다. 입력 상한 기본값은 변경 가능한 4,000 문자다. 동일 raw의 재입력은 LLM 호출 전에 중복 처리한다.
