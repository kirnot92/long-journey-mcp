...
### 5.1 입력 맥락과 observation의 단위

초기 논의에서는 관찰 하나에 가까운 raw를 제안했다. 그러나 2026-09-05 사용자가 지적했듯이 대화를 문장이나 발화 하나씩 잘라 넣으면 observation을 해석할 맥락이 사라진다. 입력의 크기와 출력 observation의 단위를 동일시하지 않는다.

대화 benchmark는 **세션의 전체 대화 → immutable Source 하나 → 문맥에서 추출한 0..N개의 depth-0 observation**으로 검증한다. 각 observation은 필요한 주체·조건·제안과 반응의 관계를 담아 독립적으로 이해할 수 있어야 한다. 모델은 전체 대화를 읽고 어느 내용을 함께 기억하고 어느 내용을 별도로 기억할지 판단한다.

같은 세션 Source에서 나온 모든 observation은 같은 root를 공유한다. observation 수가 늘어도 root 수를 늘리지 않는다. 대화 속 발언자나 프로젝트의 진위를 검증하는 정책을 추가하지 않는다.

### 5.2 실험의 입력·출력 제한

세션 입력은 날짜와 원래 순서의 모든 발화·역할을 포함한다. 질문·정답·평가용 식별자는 넣지 않는다. 원문을 자르거나 겹치는 대화 조각을 별도 Source로 만드는 방식은 사용하지 않는다.

새 benchmark의 기본값은 raw 64,000 UTF-16 문자, 최대 observation 32개다. 모두 설정할 수 있으며 개수는 목표가 아닌 상한이다. 이번 실제 실행 설정은 Remember 출력 한도를 8,192토큰으로 지정한다. 입력 초과나 API의 불완전한 출력은 성공으로 처리하지 않는다. 이 설정은 대화 benchmark에 적용하며 운영 엔진의 기본 설정과 구분한다.
