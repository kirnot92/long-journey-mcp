...
관찰할 내용이 없으면 원문만 남길 수 있으며, 하나의 Source에서 생성하는 관찰의 기본 상한은 3개입니다.
상한은 채울 목표가 아닙니다. 독립적으로 다시 찾을 가치가 있는 관찰을 선택하고,
함께 있어야 이해되는 조건·결과·예외는 한 관찰에 보존합니다.
...
| `think(topic, context?)` | 주제에 관해 축적된 철학·관점·원칙·패턴을 찾아 반환합니다. |
| `trace(memory_id)` | 기억의 부모 연결을 따라 원문 Source까지 추적합니다. |

`remember`는 명시적인 선호·제약, 중요한 결정, 관찰된 결과 또는 정정·예외처럼
이후 세션에서 참고할 내용이 갖춰졌을 때 호출합니다. 매 발화나 도구 실행마다 기록하지 않으며,
세션 종료나 맥락 압축 전에는 아직 기록하지 않은 유용한 경험을 점검합니다.
호출 에이전트가 선택한 하나의 일관된 경험에 관련 원문과 필요한 사실적 맥락을 함께 전달합니다.
중요한 표현·조건·불확실성·결과를 보존하고, 계획·결정·완료된 행동을 구분합니다.
발언자나 프로젝트 정보를 필수 인자로 요구하지 않습니다.

입력은 보통 몇 문장이나 짧은 문단으로 작성합니다. 약 500~1,500자는 참고 범위이며 최소량이나 목표가 아닙니다.
입력 상한은 기본 4,000 UTF-16 code unit이며, 도구 설명에는 실제 설정된 상한을 표시합니다.
상한을 넘으면 저장 전에 입력 오류를 반환합니다. 관련 없는 내용을 제외하되 필요한 맥락은 유지하고,
길이를 맞추려고 같은 경험을 기계적으로 나누지 않습니다. 별개의 경험은 별도로 기록합니다.
새로운 결과나 정정 없이 이미 기록한 내용을 바꾸어 다시 보내지 않습니다.
...
두 도구 모두 선택적 `context`에 현재 상황·목적·제약을 전달할 수 있습니다. 후보 검색에는 `query` 또는 `topic`만 사용하고, `context`는 후보 중 반환할 기억과 순서를 결정할 때 사용합니다.
...
서버는 초기 연결 응답에 기억을 찾고 남길 시점을 요약한 설명과 사용 지침을 제공합니다.
개별 도구 설명에는 구체적인 입력 규칙과 실제 설정 상한을 제공합니다.
일부 클라이언트는 모델에게 서버 설명만 먼저 보여주고, 도구 검색 후 개별 설명을 불러옵니다.
서버가 보낸 설명을 모델에 전달하는 방식은 클라이언트에 달려 있으므로,
별도의 서버 설명 설정을 사용하는 클라이언트에서는 그 설정에도 호출 시점을 포함해야 합니다.
자세한 구분과 설정용 문구는 [도구 발견과 설명 노출](docs/05-operations.md#도구-발견과-설명-노출)에 정리했습니다.
